...
export default function DynamicIcon({ icon, className, style }: Props) {
  const IconComponent = React.useMemo(() => {
    // Check if the icon is a valid key in LucideIcons
    if (typeof icon === 'string' && icon in LucideIcons) {
      return LucideIcons[icon as IconName] as LucideIconComponent
    }
    // If not found or invalid, return HelpCircle
    return LucideIcons.HelpCircle
  }, [icon])

  return (
    <IconComponent className={cn('size-4 shrink-0', className)} style={style} />
  )
}
